...
מתחברים פעם אחת בטלפון (הדפדפן זוכר) ורואים:
...
### יצירת משתמש להורים
Supabase → **Authentication → Users → Add user** → אימייל וסיסמה →
מסמנים **Auto Confirm User**. זהו — זה המשתמש שאיתו נכנסים ל-`/admin.html`.
אפשר ליצור משתמש לכל אחד מההורים.

> העמוד מסומן `noindex` ואינו מקושר מהדף הראשי, ורק משתמש מחובר יכול לקרוא
> את רשימת הנרשמים — הדף הציבורי יכול רק להוסיף הרשמה.
